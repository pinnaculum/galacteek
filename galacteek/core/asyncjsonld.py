...
async def compact(input_, ctx, options=None):
    """
    ...
    """
    return await JsonLdProcessor().compact(input_, ctx, options)


async def expand(input_, options=None):
    """
    ...
    """
    return await JsonLdProcessor().expand(input_, options)


async def flatten(input_, ctx=None, options=None):
    """
    ...
    """
    return await JsonLdProcessor().flatten(input_, ctx, options)


async def frame(input_, frame, options=None):
    """
    ...
    """
    return await JsonLdProcessor().frame(input_, frame, options)


async def link(input_, ctx, options=None):
    """
    **Experimental**
    ...
    if ctx:
        frame_['@context'] = ctx
    frame_['@embed'] = '@link'
    return await frame(input_, frame_, options)


async def normalize(input_, options=None):
    """
    ...
    """
    return await JsonLdProcessor().normalize(input_, options)


async def from_rdf(input_, options=None):
    """
...
async def to_rdf(input_, options=None):
    """
...
class JsonLdProcessor(object):
    ...
    async def compact(self, input_, ctx, options):
        """
        Performs JSON-LD compaction.
        ...
        if options['skipExpansion']:
            ...
            # expand input
            try:
                expanded = await self.expand(input_, options)
        ...
        try:
            active_ctx = await self.process_context(active_ctx, ctx, options)
        except JsonLdError as cause:
            raise JsonLdError(
    ...
    async def expand(self, input_, options):
        """
        Performs JSON-LD expansion.
        ...
        if _is_string(input_):
            remote_doc = await options['documentLoader'](input_)
        else:
            remote_doc = {
                'contextUrl': None,
                'documentUrl': None,
        ...
        try:
            await self._retrieve_context_urls(
        ...
        if 'expandContext' in input_:
            active_ctx = await self.process_context(
    ...
    async def flatten(self, input_, ctx, options):
        """
        Performs JSON-LD flattening.
    ...
    async def frame(self, input_, frame, options):
        """
        Performs JSON-LD framing.
        ...
        if _is_string(frame):
            remote_frame = await options['documentLoader'](frame)
        else:
            remote_frame = {
                'contextUrl': None,
                'documentUrl': None,
        ...
        try:
            ...
            expanded_frame = await self.expand(frame, opts)
        except JsonLdError as cause:
            raise JsonLdError(
    ...
    async def normalize(self, input_, options):
        """
        Performs RDF dataset normalization on the given input. The input is
        JSON-LD unless the 'inputFormat' option is used. The output is an RDF
        dataset unless the 'format' option is used'.
    ...
    async def to_rdf(self, input_, options):
        """
        Outputs the RDF dataset found in the given JSON-LD object.
    ...
    async def process_context(self, active_ctx, local_ctx, options):
        """
        ...
        if _is_object(element):
            ...
            for expanded_property, expanded_value in sorted(element.items()):
                ...
                if expanded_property == '@preserve':
                    ...
                    if not (_is_array(compacted_value)
                            and len(compacted_value) == 0):
                        JsonLdProcessor.add_value(
                ...
                if len(expanded_value) == 0:
                    ...
                    nest_result = rval
                    nest_property = active_ctx['mappings'].get(
                        item_active_property, {}).get('@nest')
                    if nest_property:
                ...
                for expanded_item in expanded_value:
                    ...
                    # if item_active_property is a @nest property, add values
                    # to nestResult, otherwise rval
                    nest_result = rval
                    nest_property = active_ctx['mappings'].get(
                        item_active_property, {}).get('@nest')
                    if nest_property:
                    ...
                    compacted_item = self._compact(
                        active_ctx, item_active_property, inner_ if (
                            is_list or is_graph) else expanded_item, options)
                    ...
                    if is_graph:
                        ...
                        if ('@graph' in container and (
                            ...
                            compacted_item = {
                                self._compact_iri(
                                    active_ctx,
                                    '@graph'): compacted_item}

                            # include @id from expanded graph, if any
                            if '@id' in expanded_item:
                                compacted_item[self._compact_iri(
                                    active_ctx, '@id')] = expanded_item['@id']

                            # include @index from expanded graph, if any
                            if '@index' in expanded_item:
                                compacted_item[self._compact_iri(
                                    active_ctx, '@index')] = expanded_item['@index']
                    ...
                    # handle language index, id and type maps
                        ...
                        map_object = nest_result.setdefault(
    ...
    def _expand(
        ...
        for key, value in sorted(element.items()):
            ...
            if expanded_property == '@type':
                ...
                types = [
                    t for t in JsonLdProcessor.arrayify(
                        element[key]) if _is_string(t)]
                for type_ in types:
                    ctx = JsonLdProcessor.get_context_value(
    ...
    def _expand_object(
        ...
        for key, value in sorted(element.items()):
            ...
            if expanded_property == '@id':
                ...
                for v in JsonLdProcessor.arrayify(value):
                    expanded_values.append(
                        v if _is_object(v) else self._expand_iri(
                            active_ctx, v, base=True))
                ...
                for v in JsonLdProcessor.arrayify(value):
                    expanded_values.append(
                        self._expand_iri(
                            ...
                            v,
                            vocab=True,
                            base=True) if _is_string(v) else v)
                JsonLdProcessor.add_value(
            ...
            if expanded_property == '@value':
                if (_is_object(value) or _is_array(
                        value)) and not options['isFrame']:
                    raise JsonLdError(
            ...
            # handle index container (skip if value is not an object)
                ...
                expanded_value = self._expand_index_map(
                    term_ctx, key, value, '@index', '@graph' in container, options)
            elif '@id' in container and _is_object(value):
                expanded_value = self._expand_index_map(
                    term_ctx, key, value, '@id', '@graph' in container, options)
            elif '@type' in container and _is_object(value):
                expanded_value = self._expand_index_map(
                    term_ctx, key, value, '@type', False, options)
            else:
            ...
            if ('@graph' in container and
                ...
                expanded_value = {
                    '@graph': JsonLdProcessor.arrayify(expanded_value)}
        ...
        for key in nests:
            for nv in JsonLdProcessor.arrayify(element[key]):
                if (not _is_object(nv) or [k for k, v in nv.items() if self._expand_iri(
                        active_ctx, k, vocab=True) == '@value']):
                    raise JsonLdError(
    ...
    def _processing_mode(self, active_ctx, version):
        ...
        if str(version) >= '1.1':
            return str(
                active_ctx.get('processingMode')) >= (
                'json-ld-' + str(version))
        else:
            return active_ctx.get(
                'processingMode',
    ...
    def _expand_value(self, active_ctx, active_property, value):
        ...
        if (type_ == '@id' or expanded_property ==
                '@graph') and _is_string(value):
    ...
    def _match_frame(self, state, subjects, frame, parent, property):
        ...
        for id_, subject in sorted(matches.items()):
            ...
            state['subjectStack'].append(
                {'subject': subject, 'graph': state['graph']})
            ...
            if '@reverse' in frame:
                for reverse_prop, subframe in sorted(
                        frame['@reverse'].items()):
        ...
        for k, v in sorted(frame.items()):
            ...
            if _is_keyword(k):
                ...
                if k == '@id':
                    ...
                    if len(
                            frame['@id']) >= 0 and not _is_empty_object(
                            frame['@id'][0]):
                ...
                if k == '@type':
                    if is_empty:
                        ...
                        return len([tv for tv in node_values if [
                                   tf for tf in frame['@type'] if tv == tf]]) > 0
            ...
            if this_frame is None:
                ...
                if _is_value(this_frame):
                    ...
                    match_this = any(self._value_match(this_frame, nv)
                        ...
                        if _is_value(list_value):
                            ...
                            match_this = any(
                                self._node_match(
                                    ...
                                    list_value,
                                    lv,
                                    flags) for lv in node_list_values)
                    else:
                        match_this = False
    ...
    def _node_match(self, state, pattern, value, flags):
        ...
        return node_object and self._filter_subject(
            state, node_object, pattern, flags)
    ...
    def _value_match(self, pattern, value):
        ...
        :param value: to check.
        """
        v1, t1, l1 = value.get('@value'), value.get(
            '@type'), value.get('@language')
        ...
        if not (v1 in v2 or _is_empty_object(v2[0])):
            return False
        if not (
            (not t1 and not t2) or (
                t1 in t2) or (
                t1 and t2 and _is_empty_object(
                t2[0]))):
            return False
        if not (
            (not l1 and not l2) or (
                l1 in l2) or (
                l1 and l2 and _is_empty_object(
                l2[0]))):
            return False
        return True
    ...
    def _compact_iri(
        ...
        if vocab and iri in inverse_context:
            ...
            containers = []
            if _is_object(
                    value) and '@index' in value and '@graph' not in value:
    ...
    def _create_term_definition(self, active_ctx, local_ctx, term, defined):
        ...
        if '@reverse' in value:
            ...
            if id_ != term:
                ...
                mapping['@id'] = id_
                mapping['_prefix'] = (
                    not _term_has_colon and re.match(
                        r'.*[:/\?#\[\]@]$',
                        id_) and (
                        _simple_term or self._processing_mode(
                            active_ctx,
                            1.0)))
        ...
        if '@container' in value:
            ...
            if self._processing_mode(active_ctx, 1.1):
                ...
                if '@list' in container:
                    ...
                    _extra_keys = [
                        kw for kw in container if kw not in [
                            '@graph', '@id', '@index', '@set']]
                    if _extra_keys:
                        raise JsonLdError(
                    ...
                    is_valid = is_valid and (
                        len(container) <= (2 if has_set else 1))
            else:  # json-ld-1.0
    ...
    async def _retrieve_context_urls(self, input_, cycles, load_document, base=''):
        """
        ...
        for url in queue:
            ...
            # recurse
            await self._retrieve_context_urls(ctx, cycles_, load_document, url)
...
def _is_graph(v):
    ...
    """
    return (_is_object(v) and '@graph' in v and len(
        [k for k, vv in v.items() if (k != '@id' and k != '@index')]) == 1)
